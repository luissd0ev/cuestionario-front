...
import {
  Contestacion,
  Pregunta,
} from '../preguntas-list/preguntas-list.interface';
  ...
  calcularValoresPonderados(): void {
    ...
    for (const prePilId in preguntasPorPilar) {
      ...
      preguntas.forEach((pregunta) => {
        if (pregunta.preTipId == 3) {
          ...
          pregunta.contestaciones.forEach((contestacion) => {
            ...
            console.log(
              'VALOR MAXIMO DE GRUPO DE PREGUNTAS::::::------------>>>>>>>>>'
            );
      ...
      this.valoresPonderadosPorPilar.push({
        ...
        valorPonderado: parseFloat(((totalEvaluadoPilar * 100) / totalPosiblePorPilar).toFixed(2)),
      });
    ...
    calculoPorcentajePonderado = parseFloat(((100 * totalEvaluado) / totalPosibleEvaluar).toFixed(2));
